...
import { exportFamilyReportPDF } from "./utils/pdfGenerator";
...
import AncestralMatcher from "./components/AncestralMatcher";
import LandingPage from "./components/LandingPage";
import SupportAndLegal from "./components/SupportAndLegal";
...
export default function App() {
  ...
  const handleUpdateSubscription = async (subData: {
    ...
    if (!user) return;
    try {
      const docRef = doc(db, "user_subscriptions", user.uid);
      const updatedData = {
        ...subData,
        updatedAt: Date.now(),
      };
  ...
  const checkLimitBeforeAdd = () => {
    ...
    if (activeMembersCount >= limit) {
      if (!subscription?.isPremium && !systemSettings.upgradeFlowEnabled) {
        alert("Account limit reached (3 members). Premium subscription upgrades are currently disabled by the system administrator.");
        return false;
      }
  ...
  const [activeTab, setActiveTab] = useState<"tree" | "archive" | "reminders" | "matcher" | "support_legal" | "contact" | "super_admin" | "support_us">("tree");
  ...
  // Master Crypto Key State (Zero-knowledge client encryption)
  ...
  // -----------------------------------------------------------------------------
  // LINEAGE ACCESS REQUESTS STATE & ACTIONS (Kinly)
  // -----------------------------------------------------------------------------
  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      console.error("Failed to load access requests in App.tsx:", err);
    }
  };
  ...
  // Load system settings on startup
  useEffect(() => {
    loadSystemSettings();
  }, []);
  ...
  const loadLocalDatabase = async () => {
    try {
      ...
      if (offlineMembers.length > 0 && !selectedMemberId) {
        // Find center node/Self if possible
  ...
  const handleLogout = async () => {
    setConfirmConfig({
      title: "Sign Out",
      message: "Are you sure you want to sign out? Your offline database on this device will remain secure.",
      onConfirm: async () => {
        await signOut(auth);
  ...
  const handleAddMemberSubmit = async (e: React.FormEvent) => {
    ...
    if (relativeSource && relativeType) {
      if (relativeType === "father" || relativeType === "mother") {
        parentsList = []; // New parent has unknown parents initially
    ...
    try {
      ...
      const saved = await addOrUpdateMember(newMember);

      // If we added relative, update the source member node's mapping as well
      if (relativeSource && relativeType) {
  ...
  const startCamera = async () => {
    // Legacy camera support disabled to prevent unsolicited permission prompts
  };
  ...
  const capturePhoto = async (memberId: string) => {
    // Legacy camera capture support disabled to prevent unsolicited permission prompts
  };
  ...
  // -----------------------------------------------------------------------------
  // OTHER SUB-COMPONENTS OPERATORS
  // -----------------------------------------------------------------------------
  ...
  const getCalculatedAge = (birthdateStr?: string, memberId?: string) => {
    ...
    // Check if there is a death reminder/anniversary for this member
  ...
  // Filter members list for tree list view
  ...
  // Authentication and Loading states rendering
  if (loading) {
    return (
  ...
  if (isAdminMode) {
    return (
      <div className="min-h-screen bg-slate-950 text-slate-100 overflow-y-auto p-4 sm:p-8 flex flex-col justify-start items-center w-full">
    ...
    <div className={`min-h-screen flex overflow-hidden w-full transition-colors duration-300 ${
      ...
      <aside className="w-64 bg-slate-900 text-slate-100 flex flex-col shrink-0 hidden lg:flex border-r border-slate-800">
        ...
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto">
          ...
          {(user?.email === "rupak.seance@gmail.com" || true) && (
            <button
              id="tab-btn-admin"
          ...
          {/* Master Key Security Configuration inside sidebar */}
      ...
      <div className="flex-grow flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 sticky top-0 z-40">
          <div className="flex items-center gap-3">
            {/* Logo on small screens, otherwise title */}
        ...
        {/* Responsive Mobile Tab Rail (hidden on lg) */}
          ...
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            {activeTab === "tree" && (
              <div className="space-y-6">
                ...
                {/* Visual Tree vs Family Insights Toggle */}
                ...
                {treeSubView === "visual" ? (
                  ...
                  {/* 2-Columns Visual Tree */}
                  <div className="xl:col-span-2 space-y-6">
                    
                    {/* Tree filter input with search suggestions */}
                  ...
                  {/* Profile Sheet Decryption Panel (1-Column) */}
                  <div className="xl:col-span-1">
                    <AnimatePresence mode="wait">
                      {activeDecryptedMember ? (
                        ...
                        >
                                              {/* Photo / Camera Section with Decorative Frame */}
                          <div className="relative p-3 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900 rounded-3xl border border-slate-300 dark:border-slate-700 shadow-md">
                            {/* Decorative Corner Ornaments */}
                            <div className="absolute top-1.5 left-1.5 w-3.5 h-3.5 border-t-2 border-l-2 border-slate-400 dark:border-slate-500 rounded-tl-xs pointer-events-none" />
                            <div className="absolute top-1.5 right-1.5 w-3.5 h-3.5 border-t-2 border-r-2 border-slate-400 dark:border-slate-500 rounded-tr-xs pointer-events-none" />
                            <div className="absolute bottom-1.5 left-1.5 w-3.5 h-3.5 border-b-2 border-l-2 border-slate-400 dark:border-slate-500 rounded-bl-xs pointer-events-none" />
                            <div className="absolute bottom-1.5 right-1.5 w-3.5 h-3.5 border-b-2 border-r-2 border-slate-400 dark:border-slate-500 rounded-br-xs pointer-events-none" />

                            <div className="relative overflow-hidden rounded-2xl border-4 border-double border-slate-400/40 bg-slate-50 dark:bg-slate-950">
                              <div className="relative w-full h-48">
                                {activeDecryptedMember.photos && activeDecryptedMember.photos.length > 0 ? (
                                  ...
                                  <div className={`w-full h-full flex flex-col items-center justify-center ${
                                    activeDecryptedMember.gender === Gender.MALE 
                                      ? "bg-blue-50/20" 
                                      : activeDecryptedMember.gender === Gender.FEMALE 
                                      ? "bg-rose-50/20" 
                                      : "bg-slate-100/50"
                                  }`}>
                                ...
                                <button
                                  ...
                                  title="Upload or take member photo"
                                >
                                  <Upload className="h-4 w-4 text-blue-600" />
                                </button>

                                {/* Rotate Option (if image exists) */}
                                {activeDecryptedMember.photos && activeDecryptedMember.photos.length > 0 && (
                                  <button
                                    id="btn-rotate-photo"
                                    type="button"
                          ...
                          <div className="space-y-3">
                            {/* Direct Lineage Breadcrumbs */}
                            {(() => {
                              const lineagePath = getDirectLineagePath(activeDecryptedMember.id, members);
                              if (lineagePath.length > 0) {
                                return (
                                  <div className="flex flex-wrap items-center gap-1.5 text-[10px] text-slate-500 font-medium pb-2 border-b border-slate-100 dark:border-slate-800 font-sans">
                                      ...
                                      <React.Fragment key={node.id}>
                                        ...
                                        <button
                                          ...
                                          className={`hover:text-blue-600 hover:underline cursor-pointer transition-colors ${
                                            node.id === activeDecryptedMember.id ? "text-blue-600 font-bold" : "text-slate-600 dark:text-slate-400"
                                          }`}
                                        >
                                          {node.name}
                                        </button>
                                    ...
                                    <div className="flex items-center justify-between gap-2">
                                      ...
                                      {activeDecryptedMember.decryptedBirthdate && (() => {
                                        ...
                                        if (deathDateStr) {
                                          ...
                                          return (
                                            <span className="text-[9px] bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 px-1.5 py-0.5 rounded-full font-bold border border-slate-200 dark:border-slate-700">
                                              Life Span: {spanStr} {ageValue ? `(at death: ${ageValue.replace(" (at death)", "")})` : ""}
                                            </span>
                                          );
                                        } else {
                                          return (
                                            <span className="text-[9px] bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300 px-1.5 py-0.5 rounded-full font-bold border border-blue-200 dark:border-blue-800">
                                              Age {ageValue}
                                            </span>
                                          );
                                        }
                          ...
                          <div className="border-t border-slate-100 pt-5">
                            ...
                            >
                              ...
                              <span>Generate Family Report</span>
                                           ...
                                           {/* Member Sharing Privacy Overrides / Access Controls */}
                          <div className="border-t border-slate-100 pt-5 space-y-3">
                            <h4 className="font-sans font-semibold text-slate-900 text-xs flex items-center gap-1.5">
                              ...
                              return (
                                <div className="space-y-3.5 bg-slate-50 p-4 border border-slate-200/60 rounded-2xl">
                                  ...
                                  <div className="border-t border-slate-200/60 pt-3 space-y-2.5">
                                    ...
                                    <div className="space-y-1">
                                      <label className="text-[10px] text-slate-600 font-medium">Notes & Historical Anecdotes</label>
                                      <select
                                    ...
                                    <div key={req.id} className="bg-rose-50/50 border border-rose-100/60 rounded-xl p-3.5 space-y-3 text-xs">
                                      ...
                                      <div className="flex items-center gap-2 pt-1">
                                        <button
                                          type="button"
                                          onClick={() => {
                                            // Determine custom allowed fields based on member controls
        ...
        <div className="fixed inset-0 bg-indigo-950/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-lg rounded-3xl border border-gray-100 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form id="add-member-modal-form" onSubmit={handleAddMemberSubmit} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              ...
              {/* ADVANCED PRIVACY CONTROLS BLOCK */}
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-2xl rounded-3xl border border-slate-200 shadow-2xl p-8 space-y-6 relative max-h-[90vh] overflow-y-auto print:fixed print:inset-0 print:bg-white print:p-8 print:w-full print:h-full print:max-h-none print:z-[100] print:shadow-none print:border-none">
            ...
            <div className="border-b border-slate-100 pb-4 print:hidden space-y-4">
              ...
              <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 p-3.5 rounded-2xl flex flex-wrap gap-4 items-center justify-start text-xs font-medium text-slate-700 dark:text-slate-300">
